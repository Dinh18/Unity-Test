...
public class BoardController : MonoBehaviour
{
    ...
    public void Update()
    {
        ...
        if (Input.GetMouseButtonUp(0))
        {
            if(m_hitCollider != null)
            {
                ...
                if(c1 != null)
                {
                    c1.ApplyItemMoveToBottom(m_board.CanMove());
                    StartCoroutine(CountCheck(0.2f));
                    StartCoroutine(CheckWin(0.2f));
                    StartCoroutine(CheckLose(1f));
                }
    ...
    private IEnumerator CountCheck(float time)
    {
        yield return new WaitForSeconds(time);
        m_board.CheckBottomCell();
    }

    private IEnumerator CheckWin(float time)
    {
        yield return new WaitForSeconds(time);
        
        if(m_board.IsWon())
        {
            m_gameManager.panelWin.GetComponent<UIPanelGameOver>().Show();
        }
    }

    private IEnumerator CheckLose(float time)
    {
        yield return new WaitForSeconds(time);
        m_gameOver = m_board.IsLosed();
        if(m_board.IsLosed())
        {
            m_gameManager.panelGameOver.GetComponent<UIPanelGameOver>().Show();
        }
    }
