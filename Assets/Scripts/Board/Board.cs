...
public class Board
{
    ...
    private BottomCell[] bottom_cells;
    private int numberOfBottomCell;
    ...
    public Board(Transform transform, GameSettings gameSettings)
    {
        ...
        numberOfBottomCell = 5;
        bottom_cells = new BottomCell[numberOfBottomCell];
    ...
    {
        Vector3 origin = new Vector3(-3,-4,0f);
        GameObject prefabBG = Resources.Load<GameObject>(Constants.PREFAB_BOTTOM_CELL_BACKGROUND);
        for(int i = 0; i < numberOfBottomCell; i++)
        {
            GameObject go = GameObject.Instantiate(prefabBG);
            go.transform.position = new Vector3(origin.x + i + 0.5f, origin.y,0);

            BottomCell btCell = go.GetComponent<BottomCell>();
            btCell.SetUp(i);
            btCell.SetItem(null);
            bottom_cells[i] = btCell;
        }
    }
    // 
    public void CheckBottomCell()
    {
        for (int i = 0; i < numberOfBottomCell; i++)
        {
            Item currentItem = bottom_cells[i].item;
            ...
            for (int j = i + 1; j < numberOfBottomCell; j++)
            {
                Item targetItem = bottom_cells[j].item;

                if (targetItem != null && currentItem.IsSameType(targetItem))
                {
                    matchIndices.Add(j);
                }
            }
            ...
            if (matchIndices.Count >= 3)
            {
                ...
                {
                    if(bottom_cells[index].item != null)
                    {
                        bottom_cells[index].item.Clear();
                        bottom_cells[index].item = null; 
                    }
                }
    ...
    public bool IsLosed()
    {
        for(int i = 0; i < numberOfBottomCell; i++)
        {
            if(bottom_cells[i].item == null) return false;
        }
        return true;
    }
    ...
    public BottomCell CanMove()
    {
        for(int i = 0; i < numberOfBottomCell; i++)
        {
            if(bottom_cells[i].isEmpty) return bottom_cells[i];
        }
        return null;
    }
